...
function formatSocialProfiles(profiles) {
    ...
    profiles.forEach(profile => {
        ...
        const textColor = profile.color || 'inherit'; // Use profile's color or default to inherit

        profilesMarkup += `<code><a href="${profile.link}" target="_blank" title="${profile.name}" style="background-color: ${profile.backgroundColor}; padding: 5px; margin: 5px; border-radius: 4px; display: flex; align-items: center; color: ${textColor};">
